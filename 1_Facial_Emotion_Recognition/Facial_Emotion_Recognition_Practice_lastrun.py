...
expName = 'facial_emotion_recognition_practice'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\jianguoyun\\1904班黄子航\\1_Facial_Emotion_Recognition\\Facial_Emotion_Recognition_Practice_lastrun.py',
...
trials = data.TrialHandler(nReps=2, method='random', 
    ...
    trialList=data.importConditions('documentation\\loop_practice.xlsx'),
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    trialComponents = [text_4, image, text_5, key_resp_face]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *text_5* updates
        if text_5.status == NOT_STARTED and tThisFlip >= 1.75-frameTolerance:
            # keep track of start time/frame for later
        ...
        if key_resp_face.status == NOT_STARTED and t >= 1.75-frameTolerance:
            # keep track of start time/frame for later
            ...
            # keyboard checking is just starting
            key_resp_face.clock.reset()  # now t=0
            key_resp_face.clearEvents(eventType='keyboard')
        if key_resp_face.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp_face.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if key_resp_face.status == STARTED:
            theseKeys = key_resp_face.getKeys(keyList=['1', '2', '3', '4', '5', '6'], waitRelease=False)
            ...
            if len(_key_resp_face_allKeys):
                ...
                # was this correct?
                if (key_resp_face.keys == str(j)) or (key_resp_face.keys == j):
    ...
    trials.addData('key_resp_face.keys',key_resp_face.keys)
    trials.addData('key_resp_face.corr', key_resp_face.corr)
    if key_resp_face.keys != None:  # we had a response
        trials.addData('key_resp_face.rt', key_resp_face.rt)
    trials.addData('key_resp_face.started', key_resp_face.tStart)
    trials.addData('key_resp_face.stopped', key_resp_face.tStop)
    thisExp.nextEntry()
